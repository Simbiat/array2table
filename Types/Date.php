...
class Date extends AbstractType
{
    ...
    protected function regular(mixed $value): string
    {
        ...
        return /** @lang HTML */ '<time id="'.$this->id.'" datetime="'.(new \DateTime($value))->format('Y-m-d').'">'.$value.'</time>';
    }
    ...
    {
        return /** @lang HTML */ '<input id="'.$this->id.'" type="date" step="1" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" value="'.$value.'">';
    }
}