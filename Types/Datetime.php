...
class Datetime extends AbstractType
{
    ...
    protected function regular(mixed $value): string
    {
        ...
        return /** @lang HTML */ '<time id="'.$this->id.'" datetime="'.new \DateTime($value)->format('c').'">'.$value.'</time>';
    }
    ...
    {
        return /** @lang HTML */ '<input id="'.$this->id.'" type="datetime-local" step="1" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}" value="'.$value.'">';
    }
}